...
app.post('/api/register', async (req, res) => {
    try {
        ...
        try {
            ...
            // Insert new user
            const userResult = await client.query('INSERT INTO users (username, email, password, is_verified) VALUES ($1, $2, $3, $4) RETURNING id', [username, email, hashedPassword, false]);
...
app.post('/api/user/goals', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const { title, description, category, target, unit, startDate, endDate } = req.body;
...
app.get('/api/verify-email', async (req, res) => {
    ...
    try {
        ...
        const tokenResult = await client.query(`SELECT user_id FROM email_verification WHERE token = $1`, [verificationToken]);
        if (tokenResult.rowCount === 0) {
        ...
        const userResult = await client.query(`SELECT id, email, username, is_verified FROM users WHERE id = $1`, [userId]);
        if (userResult.rowCount === 0) {
        ...
        const checkResult = await client.query(`SELECT is_verified FROM users WHERE id = $1`, [userId]);
...
app.post('/api/admin/challenges', authenticateAdmin, async (req, res) => {
    try {
        const { title, description, category, difficulty, carbon_reduction, points, steps, benefits, is_active } = req.body;
        // Validate required fields
...
app.put('/api/admin/challenges/:id', authenticateAdmin, async (req, res) => {
    try {
        ...
        is_active = $9
      WHERE id = $10
      RETURNING *`, [title, description, category, difficulty, carbon_reduction, points, JSON.stringify(stepsJson),
...
app.post('/api/admin/badges', authenticateAdmin, async (req, res) => {
    try {
        const { name, icon, short_description, description, requirement, category, rarity, points } = req.body;
        // Validate required fields
...
app.put('/api/admin/badges/:id', authenticateAdmin, async (req, res) => {
    try {
        ...
        points = $8
      WHERE id = $9
      RETURNING *`, [name, icon, short_description, description, requirement, category, rarity, points, badgeId]);
        if (result.rowCount === 0) {
...
app.post('/api/challenges/:id/complete', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const userChallengeResult = await client.query('SELECT * FROM user_challenges WHERE user_id = $1 AND challenge_id = $2', [userId, challengeId]);
        if (userChallengeResult?.rowCount && userChallengeResult.rowCount > 0) {
            // Update existing record
            await client.query('UPDATE user_challenges SET completed = true, progress = 100, completed_at = NOW() WHERE user_id = $1 AND challenge_id = $2', [userId, challengeId]);
        }
        else {
            // Create new record
        ...
        try {
            ...
            await client.query('UPDATE users SET points = COALESCE(points, 0) + $1 WHERE id = $2', [challenge.points, userId]);
        }
        catch (pointsError) {
  ...
  `);
            ...
            if (rank > 0 && rank <= 3) {
                ...
                const existingNotification = await client.query(`SELECT id FROM user_notifications 
       WHERE user_id = $1 
       ...
       AND reference_id = $2`, [userId, rank]);
        ...
        try {
            ...
            const challengeCountResult = await client.query('SELECT COUNT(*) FROM user_challenges WHERE user_id = $1 AND completed = TRUE', [userId]);
            ...
            for (const badge of badgesToCheck) {
                ...
                const existingBadge = await client.query('SELECT 1 FROM user_badges WHERE user_id = $1 AND badge_id = $2', [userId, badge.id]);
                ...
                if (existingBadge.rowCount === 0) {
                    ...
                    const badgeData = await client.query('SELECT name FROM badges WHERE id = $1', [badge.id]);
...
app.get('/api/leaderboard/user-rank', authenticateToken, async (req, res) => {
    ...
    `, [userPoints]);
        ...
        if (nextRankResult.rows[0]?.next_rank_points) {
            responseData.pointsToNextRank = Math.max(0, Number(nextRankResult.rows[0].next_rank_points) - userPoints);
        }
