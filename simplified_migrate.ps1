...
# Step 2: Create the econnect database if it doesn't exist
Write-Host "Making sure econnect database exists..." -ForegroundColor Green
& "$PG_BIN\psql" -h localhost -U postgres -d postgres -c "CREATE DATABASE econnect WITH OWNER = postgres ENCODING = 'UTF8' CONNECTION LIMIT = -1;" 2>$null
...
& "$PG_BIN\pg_dump" -h localhost -U postgres -d econnect -F p --no-owner --no-acl -f $BACKUP_FILE
